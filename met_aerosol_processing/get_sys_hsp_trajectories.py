...
def join_hst_sys(hst, sysfilt, maxdistance):
    ...
    pts_within = hst.sjoin_nearest(geo_sysfilt, max_distance=maxdistance, distance_col="distance").reset_index().to_crs(crs="EPSG:4326")
    # Interval between CS timestamp and trajectories points
...
df1 = pd.read_csv("/home/camilacl/git/amazon-storms-aerosols/data/hysplit_2014_backward_paths.csv", index_col=[-1])
df2 = pd.read_csv("/home/camilacl/git/amazon-storms-aerosols/data/hysplit_2015_backward_paths.csv", index_col=[-1])
...
sys_hsp = sys_filt.apply(lambda row: join_hst_sys(geo_hst_bwd, row, 25000), axis=1)

# Saving to pickle file
sys_hsp.loc[sys_hsp.name_file!=0].to_pickle("/home/camilacl/git/amazon-storms-aerosols/data/general/sysfilt_hstbwd_nearest_6h_25km.pickle")