...
"""
Main script to generate 3-week quicklooks of MIRA radar data
...
ql_files = sorted(
    glob(
        data_path + "figs/Health/Mira35_Health_Campina_*_weeks_*_*_*.png",
        recursive=True,
    )
)
if len(ql_files) == 0:
    ...
    #     "_".join(
    #         [
    #             ql_files[-1][-23:-4].split("_")[0],
    #             ql_files[-1][-23:-4].split("_")[-1],
    #         ]
    #     )
    # )
...
if ql_last == "none":
    ...
    data_week = ql_last - datetime.timedelta(days=7)
...
if len(files) < 12:
    ...
    sys.exit(
        "Not enough files - created empty plot for weeks "
        + str(
            datetime.datetime.strptime(data_ql[1], "%Y/%m/%d").isocalendar()[:2]
        )
        + " to "
        + str(
            datetime.datetime.strptime(data_ql[-1], "%Y/%m/%d").isocalendar()[
                :2
            ]
        )
    )
...
print(
    (time.time() - bt) / 60,
    "minutes to generate quicklooks for weeks ",
